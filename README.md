...
| Skill | Description |
|---|---|
| [`decomposition-driven-development`](./decomposition-driven-development) | Documentation- and contract-first methodology for building features: document the feature's observable behavior, decompose into independently testable units (pure functions, hooks, components), freeze each unit's contract + tests, graph dependencies, then implement independent units in parallel and compose. Includes `plan_waves.py` for computing parallel build waves. |
