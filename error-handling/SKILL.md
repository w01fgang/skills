...
const error = await new Try(riskyOperation, arg1, arg2).error();
...
const result = await new Try(fetchData, userId).default(null).value();
...
await new Try(syncVehicle, vehicleId, { tenantId, locationId }, retryCount)
  .breadcrumbs(["vehicleId", ["tenantId", "locationId"], "retryCount"])
...
const error = await new Try(fetchWithTimeout, url, 5000).error();
...
const result = await new Try(nestedOperation, {
  parentOperation,
  childOperation,
})
...
it("handles API failures gracefully", async () => {
  const error = await new Try(mockApiCall).error();
